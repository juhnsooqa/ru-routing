...
Раз в 2 дня и на каждый push по исходникам GitHub Actions подтягивает домены из
[v2fly/domain-list-community](https://github.com/v2fly/domain-list-community),
пересобирает `.dat`, проверяет их настоящим Xray и публикует в релиз `latest`.

Что доезжает до клиента само, а что нет:

| Что | Как обновляется |
|---|---|
| `.dat` и файлы в `dist/` | релиз `latest` и ветка `main`, раз в 2 дня |
| Страница со ссылками | пересобирается вместе с файлами, GitHub Pages подхватывает |
| Роутинг в Incy | `incy://autorouting/add/...` — приложение перечитывает ссылку само |
| Гео-файлы в Happ | приложение тянет их по `Geositeurl`, но не чаще раза в неделю |
| Профиль Happ | **сам не обновляется** — см. ниже |

Профиль Happ — это набор правил (`DirectSites`, DNS, ссылки на гео-файлы), а не
сами списки. Домены и адреса живут в `.dat` и обновляются без участия
пользователя. Профиль меняется редко, и когда это происходит, его нужно
импортировать заново: имя то же, Happ заменит старый на месте.

Чтобы не заставлять пользователей переимпортировать вручную, панель Remnawave
может отдавать профиль в заголовке подписки — для этого рядом лежит
`dist/happ-profile.b64` с готовым значением. Тогда обновлённый профиль приезжает
вместе с обновлением подписки.

`dist/version.json` — манифест сборки: дата, число доменов и sha256 каждого
файла. По нему видно, что именно сейчас в релизе, без скачивания.
